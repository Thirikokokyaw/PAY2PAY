...
export default function AdminDashboard({ onLogout }) {
  ...
  const [newAdmin, setNewAdmin] = useState({ name: '', email: '', phone: '', role: 'Support' });
  ...
  const handleCreateAdmin = (e) => {
    e.preventDefault();
    if (!newAdmin.name || !newAdmin.email || !newAdmin.phone) return;
    setAdmins(prev => [...prev, { ...newAdmin, id: Date.now(), status: 'Active' }]);
    setNewAdmin({ name: '', email: '', phone: '', role: 'Support' });
  };
    ...
    <div className={`flex flex-col md:flex-row min-h-screen font-sans antialiased transition-colors duration-300 ${theme.bg}`}>
      ...
      <aside className={`hidden md:flex flex-col justify-between shrink-0 transition-all duration-300 border-r relative ${theme.sidebar} ${isSidebarCollapsed ? 'w-20' : 'w-72'}`}>
        ...
        <div className={`p-3 border-t ${isDarkMode ? 'border-slate-800 bg-slate-950/40' : 'border-slate-200 bg-slate-50'}`}>
          ...
          <button onClick={onLogout} className={`w-full py-2 border rounded-xl text-xs font-bold transition-all text-center ${isSidebarCollapsed ? 'px-0 text-[10px]' : ''} ${isDarkMode ? 'bg-slate-800 hover:bg-rose-950/40 border-slate-700 text-slate-300 hover:text-rose-400' : 'bg-white hover:bg-rose-50 border-slate-200 text-slate-600 hover:text-rose-600 shadow-sm'}`}>
