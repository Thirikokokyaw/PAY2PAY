      ...
      *==== [GLOBAL VALIDATION] CHECK JAPANESE CHARACTERS ====
           PERFORM VARYING I FROM 1 BY 1 UNTIL I > 50
               IF WS-NAME(I:1) > X"7F" OR 
                  WS-EMAIL(I:1) > X"7F" OR
                  WS-PASSWORD(I:1) > X"7F"
      ...
      *==== [ACTION: REGISTER OR UPDATE] PHONE & EMAIL CHECK ====
           IF WS-ACTION = 'REGISTER' OR WS-ACTION = 'UPDATE'
               MOVE 0 TO PHONE-LEN
      ...
      *==== [ACTION: SUPPORT] TRANSACTION ID CHECK ====
           IF WS-ACTION = 'SUPPORT'
               IF WS-PHONE(1:4) NOT = 'TXN-' AND 
                  WS-PHONE NOT = 'N/A' AND WS-PHONE NOT = SPACES
                   MOVE 'ERROR' TO WS-STATUS
                   MOVE 'Invalid Transaction ID Format.'
                       TO WS-MSG
                   PERFORM DISPLAY-AND-EXIT
               END-IF
           END-IF.
