    ...
    try {
      if (typeof setAdminData === 'function') {
        const finalPayload = {
          ...
          avatar: editForm.avatar || adminData.avatar,
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
          ...
          <div className={`relative w-full max-w-md p-5 sm:p-6 rounded-2xl border shadow-2xl transition-all animate-in zoom-in-95 duration-150 ${isDarkMode ? 'bg-slate-900 border-slate-800 text-white' : 'bg-white border-slate-200 text-slate-900'}`}>
            <div className="flex items-center justify-between pb-3 mb-4 border-b border-slate-500/10">
              ...
              <button type="button" onClick={() => { setIsPasswordModalOpen(false); setOldPasswordError(''); setNewPasswordError(''); setConfirmPasswordError(''); }} className={`p-1.5 rounded-lg border transition-all cursor-pointer ${isDarkMode ? 'border-slate-800 hover:bg-slate-800 text-slate-400' : 'border-slate-200 hover:bg-slate-50 text-slate-500'}`}><X size={16} /></button>
            </div>
            <form onSubmit={handlePasswordSubmit} className="space-y-4">
              ...
              <div className="pt-2 flex justify-end gap-2">
                <button type="button" onClick={() => { setIsPasswordModalOpen(false); setOldPasswordError(''); setNewPasswordError(''); setConfirmPasswordError(''); }} className={`px-4 py-2 text-xs font-bold rounded-xl border cursor-pointer ${isDarkMode ? 'border-slate-800 hover:bg-slate-800 text-slate-400' : 'border-slate-200 hover:bg-slate-50 text-slate-500'}`}>Dismiss</button>
